...
che_data <- top_countries %>%
  filter(country == "CHE")

che_hotelbookings <- che_data %>%
  group_by(hotel) %>%
  summarise(n = n()) %>%
  mutate(percentage = n / sum(n) * 100)

ggplot(che_hotelbookings, aes(x = hotel, y = n, fill = hotel)) + 
